    ...
    async execute(interaction) {
        ...
        if (interaction.isButton()) {
            ...
            if (buttonId.startsWith("get_otp_")) {
                await interaction.reply({
                    content: "🔑 Accediendo al webmail y buscando el código OTP... esto puede tardar unos segundos.",
                    ephemeral: true,
                });
                ...
                if (result.success) {
                    ...
                    await logAction(interaction.client, `🔑 **${interaction.user.tag}** ha obtenido un código OTP para la cuenta \`${email}\`.`);
                } else {
            ...
            if (buttonId.startsWith("get_2fa_")) {
                ...
                try {
                    ...
                    if (otpCode) {
                        ...
                        await logAction(interaction.client, `🔑 **${interaction.user.tag}** ha obtenido un código 2FA.`);
                    } else {
                        await interaction.editReply(`❌ **Error:** La API no devolvió un código válido.`);
                    }
                } catch (error) {
                    console.error("Error al llamar a la API de 2FA:", error.message);
                    await interaction.editReply(`❌ **Error:** No se pudo conectar con la API de 2FA. (Status: ${error.response ? error.response.status : "N/A"})`);
                }
            }
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (modalId === "add_accounts_modal") {
                ...
                await logAction(interaction.client, `➕ **${interaction.user.tag}** ha añadido **${result.added}** cuentas nuevas a **${category}**.`);
            }
